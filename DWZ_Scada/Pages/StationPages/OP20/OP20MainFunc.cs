...
namespace DWZ_Scada.Pages.StationPages.OP10
{
    ...
    {
        private static readonly OP20Model myOp10Model = new();

        public static string StationName = "OP20";
        ...
        private async Task ProcessEntrySignal(DateTime dt)
        {
            ...
            {
                PLC.Read(OP20Address.EntrySn, "string", out string sn);
                OP20EntryCommand entryCommand = new(sn);
                entryCommand.Execute();

                if (PLC.ReadInt16(OP20Address.Collect, out int collectSignal) && collectSignal == 1)
                {
                    //TODO 开始数据采集
                    if (IsSpotCheck)
                    {
