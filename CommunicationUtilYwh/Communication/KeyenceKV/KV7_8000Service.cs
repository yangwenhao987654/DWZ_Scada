...
{
     public class KV7_8000Service:IDisposable
    {
        Socket KVTCPClicent;
        ...
        public bool ReadBool(string adr, out bool value)
        {
            ...
            try
            {
                string result = Get(adr, DataType.N);
                bool parseResult = int.TryParse(result, out int v);
                if (v==1)
                {
                    value = true;
                }
                ...
                if (!parseResult)
                {
                    LogMgr.Instance.Error($"读取Bool失败:返回[{result}] 地址:{adr} ");
                }
            }
            catch (Exception ex)
            {
                LogMgr.Instance.Error($"KVServer Read错误,地址:[{adr}] 类型[Bool] 异常信息:{ex.Message}");
                flag = false;
            }
            return flag;
        }
        ...
        public  bool ReadAlarm(string adr, out bool[] value, int length)
        {
            ...
            try
            {
                string result = Get(adr, (ushort)length,DataType.N);
                MyPlc.RemoveAllCharactersAfterBackslashOrNull(result);
                //LogMgr.Instance.Debug($"读取地址[{adr}] 长度:[{length}] 结果:{result}");
                if (result.Length > 0)
                {
            ...
            {
                LogMgr.Instance.Error($"PLC Read错误,地址:[{adr}] 类型[Bool-{length}] 异常信息:{ex.Message} 堆栈{ex.StackTrace}");
                flag = false;
            }
            return flag;
        }
